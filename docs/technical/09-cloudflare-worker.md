...
`scheduled(event, env)` ([worker.js:2774](../../cloudflare-worker/worker.js#L2774)):
...
Маршрутизатор — `fetch(request, env)` ([2817](../../cloudflare-worker/worker.js#L2817)).
...
| `/admin/visits` | GET | `handleAdminVisits` ([2692](../../cloudflare-worker/worker.js#L2692)) | `OWNER_SECRET` | Сводка посещений одним KV-list: дни, итоги, список устройств. |
