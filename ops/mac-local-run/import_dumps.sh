...
# С 09.09.2026 запись берётся не только слотами: ops/vps-run/import_poll.sh
# каждые 5 мин читает флаг «есть новое» и зовёт этот скрипт по клону сразу.
...
sber_preflight() {  # 0 — можно идти в суды
  ...
  # пути — текст алерта). Проба мульти-хост (20.08.2026,
  # cm_any_court_reachable): sudrf «мигает» пер-хостово, и одиночный хост
  # давал ложный отказ на весь импорт. С 25.08 проверяем реальные страницы
  # поиска/карточки, поэтому stdout — уже сводка, а не имя одного суда.
...
# любой сети (на sudrf он не ходит), а сетевая проба судов при пустой очереди
...
courts_gate() {
  ...
  # запрашивались вовсе.
  export CARD_BREAKER_MODE="${CARD_BREAKER_MODE:-count}"
...
# Пометка «повтор не поможет» в тексте ошибки сводки: очередь
# (import_queue.jq, NO_RETRY_MARK) такую failed-запись больше не берёт.
# Текст ОДИН на скрипт и jq — менять парой (страж test_mac_import_dumps.py).
NO_RETRY_MARK="повтор не поможет — вставьте выдачу заново"
mark_no_retry() {  # $1 = summary.json
  jq --arg m "$NO_RETRY_MARK" \
     '.error = ((.error // "ошибка импортёра") + " · " + $m)' \
     "$1" > "$1.tmp" && mv "$1.tmp" "$1"
}
...
run_import() {  # $1 = файл дампа, $2 = домен суда, $3 = оператор, $4 = ключ|""
  ...
  status=done
  if [ "$rc" -ne 0 ]; then
    # Любой ненулевой код импортёра — вердикт о САМОМ дампе (2 код на
    # странице, 3 таблицы нет, 4 суд не в реестре, 5 чужой суд): повтор с
    # тем же телом даст то же самое. Маркер в тексте ошибки — гейт очереди
    # (import_queue.jq не берёт failed с ним): без него запись крутилась
    # каждым слотом до истечения TTL (08.09.2026: три «текстовых» дампа Урала
    # × 5 слотов в день, сервис каждый слот «failed»).
    status=failed
    mark_no_retry "$summary"
  elif [ "$DRY_RUN" = "1" ]; then
...
run_add_cases() {  # $1 = файл задания, $2 = ключ|""
  ...
  if [ "$rc" -ne 0 ]; then
    status=failed
    # 5 = задание нечитаемо — повтор бесполезен; 4 = тотальный сетевой сбой —
    # следующий слот возьмёт снова.
    [ "$rc" -eq 5 ] && mark_no_retry "$summary"
  elif [ "$DRY_RUN" = "1" ]; then
    log "  DRY-RUN: коммит и отчёт пропущены"
    return 0
...
# Работа точно есть — теперь можно идти к судам (маршруты, реальная проба, git).
