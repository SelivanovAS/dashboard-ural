...
for repo in "${repos[@]}"; do
  ...
  echo "  → $repo"
  # ⚠️ Лок ПОКЛОНОВЫЙ, и занятая территория цикл НЕ прерывает: воркер
  # тихо выходит (exit 0), а слот идёт к следующему клону. 24.08.2026
  # слот 08:00 так подхватил Урал параллельно ручному прогону ХМАО —
  # поведение полезное (территории независимы), но неочевидное.
