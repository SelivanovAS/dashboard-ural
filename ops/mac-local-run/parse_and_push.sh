...
# утренних карточек). С той же семантикой пропускаются и ПОИСКИ судов, чья
# выдача сегодня уже дала строки (searched_ok_today, health.py): слепой
# WAF-ноль успехом не считается и ретраится каждым слотом, капчёвая
# апелляция не пропускается никогда. --force гасит всё: юрист у пульта
# хочет полный свежий прогон.
#
