...
# Решение юриста 20.08.2026: слоты 08:00–10:00 копят новости молча (черновые
# коммиты без маркера), отправку делает ЗАМЫКАЮЩАЯ попытка — удачная (поиски
# зрячие И карточки ≥85% плана) либо дедлайн 10:00 с тем, что накопилось.
...
# (08:00–10:00), и сорвавшуюся пробу добивает следующий (20.08.2026 Урал не
# прошёл в 08:19 и спарсился в 08:30), а алерт на каждую неудачу дал 5
# одинаковых сообщений за утро. Тихий выход до дедлайна; на дедлайне (слот
# 10:00 — последний, либо Mac проснулся после сна): накоплено что-то за утро →
# доставка накопленного БЕЗ парсинга, иначе — ОДИН алерт в день «утро
# потеряно» (маркер-файл .alerted-parse-ДАТА, живёт рядом с логами, в git не
# попадает). Ручной запуск (--force) кричит сразу — юрист смотрит на экран.
DELIVERY_DEADLINE_MIN=595   # 09:55 местного: слот 10:00 — последний, шлём что есть
past_deadline() {
  [ $(( 10#$(date +%H) * 60 + 10#$(date +%M) )) -ge "$DELIVERY_DEADLINE_MIN" ]
}
...
probe_failed() {  # $1 = диагностика канареек; наружу не возвращается
  ...
  if past_deadline; then
    if "$PYTHON" ops/mac-local-run/cloud_run_ok.py --has-pending >/dev/null 2>&1; then
      # Утро что-то накопило черновыми попытками — дедлайн отправляет это
      # без парсинга: доставочный коммит несёт один штамп delivered_at.
      alert_telegram "суды к дедлайну так и не ответили — отправляю дайджест с накопленным: $("$PYTHON" ops/mac-local-run/cloud_run_ok.py --progress 2>/dev/null || echo 'без сводки')"
...
# Удачная попытка (--run-complete читает СВЕЖИЙ журнал этого прогона: поиски
# зрячие И карточки ≥85% плана), дедлайн 10:00 или --force → доставка: штамп
# delivered_at + маркер «(Mac-парсинг)» → replay шлёт ОДИН дайджест со ВСЕМ
# накопленным контекстом. Иначе — черновой коммит: данные и накопление
# публикуются (дашборд свежий), replay молчит, алерт-прогресс говорит юристу,
# сколько дочиталось (решение юриста 20.08.2026 — после каждой попытки).
...
if git push "$GIT_URL" HEAD:main >>"$LOG" 2>&1; then
  if [ "$DELIVER" = "1" ]; then
    ...
    if [ "$RUN_OK" != "1" ] && [ "$FORCE" != "1" ]; then
      # Дедлайн с неполной попыткой: юрист должен знать, что дайджест неполный.
      alert_telegram "дедлайн: дайджест отправлен с тем, что дочиталось — $("$PYTHON" ops/mac-local-run/cloud_run_ok.py --progress 2>/dev/null || echo 'без сводки')"
    fi
  else
    log "Черновик запушен: ${RUN_WHY:-вердикт не прочитался}; дайджест копится"
    notify "Попытка неполная — копим, следующий слот через полчаса"
    # Алерт-прогресс после КАЖДОЙ неполной попытки — решение юриста 20.08.2026.
    alert_telegram "попытка неполная: $("$PYTHON" ops/mac-local-run/cloud_run_ok.py --progress 2>/dev/null || echo 'без сводки') — пробую ещё (слоты каждые полчаса до 10:00)"
  fi
else
