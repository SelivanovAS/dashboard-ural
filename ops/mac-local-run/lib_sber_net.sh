...
# ── Окно единственной дневной доставки ───────────────────────────────────────
# Одна граница на parse_and_push.sh и родительский parse_all.sh. Иначе воркер
# мог бы считать, что 08:45 уже наступило, а финальный sweep — что ещё нет (или
# наоборот), и первая территория снова осталась бы с pending-контекстом.
# Override нужен детерминированным shell-тестам; в бою значение не задаётся.
CM_DELIVERY_WINDOW_MIN="${CM_DELIVERY_WINDOW_MIN:-525}"  # 08:45 местного
case "$CM_DELIVERY_WINDOW_MIN" in
  ""|*[!0-9]*) CM_DELIVERY_WINDOW_MIN=525 ;;
  # 1440 — разрешённое «никогда сегодня» для детерминированных shell-тестов.
  *) [ "$CM_DELIVERY_WINDOW_MIN" -le 1440 ] || CM_DELIVERY_WINDOW_MIN=525 ;;
esac

cm_delivery_window_open() {
  local hour minute now
  hour=$(date +%H)
  minute=$(date +%M)
  now=$((10#$hour * 60 + 10#$minute))
  [ "$now" -ge "$CM_DELIVERY_WINDOW_MIN" ]
}
