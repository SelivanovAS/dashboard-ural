...
def _check_section_counters(html: str) -> list[str]:
    ...
    ⚠️ Строка может нести НЕСКОЛЬКО дел: свёртка «вступило в силу»
    (`_bank_force_fold_line`, 21.08.2026) перечисляет свои дела ссылками в
    одну строку. Поэтому считаем ОБЁРНУТЫЕ номера (`_ANCHOR_RE`), а не
    строки, с полом в единицу. Считать все номера подряд нельзя: строка
    новой апелляции несёт номер 1-й инстанции ХВОСТОМ, голым — он дал бы
    двойной счёт. Обратная совместимость проверена на боевых выпусках обеих
    территорий 21.08.2026: во ВСЕХ строках ровно один обёрнутый номер
    (ХМАО 32, Урал 93), то есть для прежнего формата поведение не меняется."""
    problems: list[str] = []
    lines = html.split("\n")
    ...
    for ln in lines:
        ...
        if current_header is not None and _line_has_case_number(ln):
            counted += max(1, len(_ANCHOR_RE.findall(ln)))
    _flush()
    return problems
