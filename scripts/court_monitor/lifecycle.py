...
# Коды причин ручной пометки «лист не нужен» (решение юриста 21.08.2026).
# Список закрытый: юрист выбирает из трёх вариантов, а не пишет текст — так
# потом можно посчитать, сколько дел закрывается добровольным погашением.
WRIT_WAIVE_REASONS = {
...
def writ_waive_reason_ru(fi: dict) -> str:
    """Человеческая причина пометки или пустая строка."""
    code = ((fi or {}).get("writ_waived") or {}).get("reason") or ""
    return WRIT_WAIVE_REASONS.get(code, "иное" if code else "")


def bank_writ_waived(fi: dict) -> bool:
    """Помечено ли дело человеком как «листа не будет».

    Ручной канал (админка → KV → workflow → cases_bank.json). Нужен потому,
    что добровольное погашение долга после решения из карточки суда НЕ видно
    вовсе: проверка 16 287 событий обеих территорий дала ноль вхождений слов
    «погаш»/«добровольн»/«исполнительн» — ГАС «Правосудие» сведений об
    исполнении не публикует. Кейс, с которого началось: уральское 2-28/2026
    ждало лист 171 день, а лист не выдадут — ответчик заплатил.

    ⚠️ Пометка живёт ОТДЕЛЬНЫМ ключом, а не в `writ_expected`: тот
    пересчитывается каждым прогоном (`split_bank_track`) и ручную правку
    затёр бы. Пустой `reason` пометкой не считается — так снятие пометки
    можно записать пустым словарём, не удаляя ключ.
    """
    return bool(((fi or {}).get("writ_waived") or {}).get("reason"))


# «Дело передано в архив» — суд закрыл производство и сдал дело в хранилище.
# ⚠️ Не путать с «Дело сдано в отдел судебного делопроизводства»: так
# заканчивается 73 из 151 дела очереди ожидания, это рутинная канцелярия.
_FI_COURT_ARCHIVED_RX = re.compile(
    r"дел\w*\s+передан\w*\s+в\s+архив", re.IGNORECASE)


def bank_court_archived_date(fi: dict) -> str:
    """Дата, когда СУД сдал дело в архив, если листа на исполнение так и нет.

    Косвенный признак «лист не запрашивали»: суд закрыл дело, а исполнительный
    лист не выдан. Подсказка для админки — юрист подтверждает её кнопкой
    (решение 21.08.2026: автоматикой не помечаем, лист могут выдать позже по
    заявлению взыскателя).

    Проверено на боевых данных обеих территорий: среди ждущих лист признак
    дают 4 дела (все на Урале), ложных срабатываний нет. Единственный
    контрпример — ХМАО 2-248/2026, где дело сдано в архив в ТОТ ЖЕ ДЕНЬ, что
    выдан лист, — сюда не попадает как раз из-за проверки листов.
    """
    fi = fi or {}
    if any(classify_writ_kind(w, fi) == "enforcement"
           for w in fi.get("writs") or []):
        return ""
    for ev in reversed(fi.get("events") or []):
        # Колонка `name` точнее (в `text` склеены все ячейки), но 43% событий
        # приходят legacy-склейками без колонок — отсюда фолбэк.
        if _FI_COURT_ARCHIVED_RX.search(
                (ev.get("name") or "") or (ev.get("text") or "")):
            return (ev.get("date") or "").strip()
    return ""


def bank_writ_awaited(fi: dict) -> bool:
    """Ждём ли мы по этому делу исполнительный лист ПРЯМО СЕЙЧАС.

    Судебные причины «листа не будет» (`bank_writ_expected`) плюс ручная
