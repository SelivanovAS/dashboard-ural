...
FI_ARCHIVE_DAYS = 60            # 1-я инстанция, статус «Возвращено»: 60 дней
                                # от даты события/заседания без жалобы → архив.
                                # У РЕШЁННЫХ дел с 04.09.2026 окно считает
                                # fi_appeal_window_end (lifecycle.py) по ГПК:
                                # мотивировка + месяц (ст. 321/108) +
                                # FI_APPEAL_GRACE_DAYS; без мотивировки в
                                # карточке — резолютивка + 10 раб. дн (ст. 199)
                                # + месяц + запас ≈ те же 60 дней. Прежние
                                # плоские 60 от заседания хоронили дело раньше
                                # срока на жалобу, когда суд задерживал
                                # мотивировку (2-857/2026: заседание 25.06,
                                # мотивировка 10.08 → срок 10.09, архив 25.08).
                                # Константа остаётся полом реактивации
                                # (reactivate_archived_first_instance ≤180),
                                # гейтом discovered_already_resolved_old и
                                # фронтовым ARCHIVE_DAYS. Архив не финален: при
                                # появлении жалобы дело возвращается в активные.
FI_APPEAL_GRACE_DAYS = 14       # Запас ПОСЛЕ месячного срока на апел. жалобу
                                # (решение юриста 04.09.2026): жалоба, сданная
                                # на почту в последний день, и её регистрация
                                # судом видны в карточке с опозданием. Общий
                                # для обеих картотек (основная и «Иски банка»).
...
BANK_WRIT_OVERDUE_ALERT_DAYS = 30  # алерт дайджеста «ИЛ не выдан N дн. после
                                   # вступления в силу» (fi_writ_overdue,
                                   # календарный проход collect_bank_calendar_
                                   # events). Порог синхронен первому порогу
                                   # фронтового бейджа «⏳ ждёт ИЛ» (30/60 дн в
...
BANK_CALENDAR_EVENTS_SINCE = date(2026, 8, 13)
                                ...
                                # события эмитятся только для дат, наступивших
                                # ПОСЛЕ этой даты; более ранние тихо получают
                                # маркер. Это анти-паводок деплоя (решение
                                # юриста 13.08.2026: бэклог «64 в силе /
                                # 14 просрочено» не объявлять — он виден в
                                # чипе «Ждут ИЛ» дашборда). Посевом в
                                # migrate_stages это не решается: расчётная
                                # дата пересекает «сегодня» без изменения
                                # данных, и посев на загрузке (раньше прохода)
                                # глушил бы и все будущие события.
...
BANK_DEFAULT_CANCEL_DAILY_GRACE_DAYS = 14  # pending-отмена заочного БЕЗ даты
                                     # заседания (или с прошедшей): столько дней
                                     # от якоря (дата заседания, иначе дата
                                     # подачи заявления) карточка читается
                                     # ежедневно — суд назначает/рассматривает
                                     # за 10 дн (ст. 240); дальше — ритм
                                     # BANK_WRIT_CHECK_DAYS, иначе зависшее
                                     # дело читалось бы каждым прогоном все
                                     # 90 дн потолка (кейс 2-3005/2026 Урала).
...
# В иске ОТКАЗАНО / листа не будет → ждём только апел. жалобу банка. С
# 04.09.2026 окно ОБЩЕЕ с основной картотекой — fi_appeal_window_end
# (мотивировка + месяц ст. 321 + FI_APPEAL_GRACE_DAYS); прежняя
# BANK_DENIED_ARCHIVE_DAYS=30 от мотивировки шла без запаса на почту.
...
# Потолок карточек-кандидатов на ОДИН суд. Фаза поиска 1-й инст. идёт
# раньше FI-цикла карточек, и
...
# Столько заведённых за прогон исков дайджест печатает подельно; больше —
# одной свёрнутой строкой «заведено N новых исков банка». Разгон Урала
# 14.08.2026: первый прогон авто-подхвата дал 116 одинаковых строк «взят на
# мониторинг» (HTML 60 КБ), и настоящие события — решения, ИЛ, заседания —
# в них утонули. 0 — не сворачивать (пилот, отладка).
BANK_INTAKE_DIGEST_FOLD = int(
    os.environ.get("BANK_INTAKE_DIGEST_FOLD", "25") or 25
)
# Столько дел «✅ решение вступило в силу — ожидаем ИЛ» дайджест печатает
# отдельными строками; больше — одной строкой со списком номеров (просьба
# юриста 21.08.2026). Событие КАЛЕНДАРНОЕ: наступает по расчёту, а не по
# движению дела, и на дашборде эти дела и так видны бейджем «⏳ ждёт ИЛ».
# Замер 21.08.2026: 14 строк из 80 у Урала (16% секции) и 8 из 26 у ХМАО —
# самый массовый рутинный класс на ОБЕИХ территориях. Порог низкий: две-три
# строки читаются нормально, а список экономит место с четвёртой.
# 0 — не сворачивать.
BANK_FORCE_DIGEST_FOLD = int(
    os.environ.get("BANK_FORCE_DIGEST_FOLD", "3") or 3
)
# Потолок перечня сторон в КОМПАКТНОЙ строке банк-секции дайджеста: длиннее
# — печатаем первые DIGEST_PARTIES_KEEP сторон и «и ещё N» (решение юриста
# 21.08.2026). Замер 21.08: 17 перечней длиннее 60 симв. при медиане 24,
# рекорд — 238 (пять соответчиков наследственного дела), и такие строки
# ломали ритм чтения. Полный состав остаётся в карточке дела на дашборде.
# ⚠️ Потолок живёт в рендере, а НЕ в shorten_party_name: её результат уходит
# в case_meta LLM-пересказчика, и «и ещё 3» вместо имён испортило бы пересказ;
# в секциях 3.5/3.6/5.5 юрист просил стороны целиком. 0 — не резать.
DIGEST_PARTIES_MAX_LEN = int(
    os.environ.get("DIGEST_PARTIES_MAX_LEN", "60") or 60
)
DIGEST_PARTIES_KEEP = int(os.environ.get("DIGEST_PARTIES_KEEP", "2") or 2)
...
# Дочитка утренних слотов Mac-резерва («один дайджест в день»): карточка со
# штампом last_checked_at за СЕГОДНЯ пропускается — повторный слот тратит
# дефицитные при блоке запросы только на недочитанное. Ставит ТОЛЬКО
# parse_and_push.sh (и гасит при --force — юрист у пульта хочет полный свежий
# прогон); облачный workflow и ручные прогоны переменную не передают. Урок
# 21.08.2026: второй слот Урала из 119 удачных чтений потратил ~105 на повторы
# утренних карточек, пока ~130 недочитанных остались ждать завтра.
SKIP_CHECKED_TODAY = os.environ.get("SKIP_CHECKED_TODAY", "0") == "1"
# Горизонт доверия к будущей дате из карточки суда (дни). Дальше него дата
# известной не считается, и 21-дневная страховка force-parse снова работает —
# см. асимметрию в should_skip_case. Причина: даты приходят от суда как есть, а
# 2-1725/2026 приехало с заседанием 20.08.2029 при соседних событиях от
# 29.07.2026 (опечатка в годе) и молчало бы три года. Порог с двукратным
# запасом: максимум законной дистанции по обеим территориям — 90 дней, между
# 90 днями и годом нет ни одного дела. Классические долгие приостановки сюда не
# попадают — get_next_planned_date матчит «без движения», а не «производство
# приостановлено».
KNOWN_DATE_TRUST_DAYS = int(os.environ.get("KNOWN_DATE_TRUST_DAYS", "180"))
...
# `raise_for_status` внутри fetch_page. Импорт трёх судов Свердловской области
# 16.08 не прочитал ни одной карточки, и понять «нас блокируют по адресу» или
# «портал лёг» было нечем. Жизненный цикл — один прогон, как у CARD_BREAKER.
FETCH_DIAG: dict = {}

# ── Наблюдаемость сети: сколько шло и почему не вышло ────────────────────────
# 24.08.2026 за одно утро случились ДВА разных отказа портала — «отвечает за
# 26–58 с при таймауте 30 с» и «отдаёт заглушку с HTTP 200», — и оба пришлось
# разбирать вручную curl'ом: система знала, что запрос не удался, но не знала
# ни СКОЛЬКО он шёл, ни ПОЧЕМУ. Лечатся эти режимы по-разному (поднять таймаут
# / ждать ГАС / флипнуть канал), так что различать их надо по данным.
# ⚠️ Живут ОТДЕЛЬНО от METRICS: тот объявлен dict[str, int], а _metrics_reset
# присваивает каждому его ключу 0 — ни список, ни вложенный счётчик там не
