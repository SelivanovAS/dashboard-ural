...
def _warn_if_card_degraded(
    ...
    Возвращает "" для нормальной карточки, для «огрызка» — вид:
    "degraded" (warning + метрика) или "suspended" (ожидаемый огрызок
    дела «без движения», debug). Обе непустые строки truthy — пер-кейсовый
    отчёт bank-трека помечает degraded по прежней truthiness; апел-цикл
    различает виды: "degraded" не считается прочитанной карточкой (без
    бампа last_checked_at), а "suspended" обязан бампать — на
    last_checked_at живёт недельный ритм suspended_weekly.
    """
...
def parse_case_card(html: str, court_base_url: str = "") -> dict:
    ...
    info = {
        ...
        "Номер материала (карточка)": "",  # М-номер из того же заголовка («… ~ М-…» или «ДЕЛО № М-…»)
        # Дата поступления и категория из таблицы «ДЕЛО»: нужны точечному
    ...
    if _title_m:
        ...
        # (?<![\dМ-]) — гражданский номер не должен быть хвостом М-номера:
        # внутри материала Покачи «М-2-309/2026» буквально сидит «2-309/2026»,
        # и без гарда карточка одного материала отдавала бы ложный гражданский
        # номер. Средний сегмент — постоянное присутствие («2-2-279/2026»).
        _first = re.search(
            r"(?<![\dМ-])(\d+-(?:\d+-)?\d+/\d{4})|(М-(?:\d+-)?\d+/\d{4})",
            _seg)
        if _first and _first.group(1):
            info["Номер дела (карточка)"] = _first.group(1)
        # М-номер из того же заголовка — отдельным ключом: точечное добавление
        # по ссылке промоутит существующую М-запись вместо создания дубля.
        # Средний сегмент — постоянное присутствие (Покачи: «М-2-309/2026»).
        _mat = re.search(r"М-(?:\d+-)?\d+/\d{4}", _seg)
        if _mat:
            info["Номер материала (карточка)"] = _mat.group(0)
    ...
    for tbl_idx in range(len(tables)):
        ...
        for row in tbl:
            ...
            # Дата поступления и категория дела — явные строки таблицы «ДЕЛО»
            # карточки 1-й инстанции. Точечное добавление по ссылке собирает
            # запись без строки выдачи — этим полям больше взяться неоткуда.
            if (label_l.startswith("дата поступления")
                    and not info["Дата поступления (карточка)"]):
                _date_m = re.search(r"\d{2}\.\d{2}\.\d{4}", value)
                if _date_m:
                    info["Дата поступления (карточка)"] = _date_m.group(0)
            if (label_l.startswith("категория")
                    and not info["Категория (карточка)"]):
                if value and value.lower() != label_l:
                    info["Категория (карточка)"] = value
