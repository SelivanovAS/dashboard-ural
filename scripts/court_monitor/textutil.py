...
_MTU_HEAD = (
    ...
    #
    # Голов ЧЕТЫРЕ (21.08.2026): кроме межрегионального управления бывает
    # обычное территориальное — форма зависит от субъекта, и уральские
    # «Территориальное управление Федерального агентства по управлению
    # государственным имуществом по Свердловской области» (114 символов!) и
    # «ТУ Росимущества по Свердловской области» не сокращались ВОВСЕ: 13
    # вхождений в боевом выпуске 21.08.2026. Падежи через \w+ — названия
    # приходят и в родительном («Межрегионального территориального
    # управления»).
    #
    # ⚠️ \b перед МТУ/ТУ load-bearing: _MTU_FULL_RE применяется через .sub()
    # (матч в ЛЮБОЙ позиции), и без границы слова «МТУ Росимущества» совпало
    # бы с офсета 1 как «ТУ Росимущества», оставив осиротевшую «М».
    r'(?:\bМежрегиональн\w+\s+территориальн\w+\s+управлени\w+'
    r'|\bТерриториальн\w+\s+управлени\w+'
    r'|\bМТУ\b|\bТУ\b)\s+["«]?\s*'
    r'(?:(?:Федеральн\w+\s+агентств\w+|ФА)\s+по\s+управлению\s+'
...
def _rosim_short(m) -> str:
    """«МТУ Росимущество» / «ТУ Росимущество» — по голове найденного названия.

    Межрегиональное управление и обычное территориальное — РАЗНЫЕ органы
    (первое обслуживает несколько субъектов, второе один), поэтому в одно имя
    их не схлопываем: юрист по нему понимает, с кем спорит. Функция —
    replacement для .sub(), где раньше стояла строка-константа.
    """
    head = m.group(0).lstrip('«"').strip().lower()
    interregional = head.startswith("меж") or head.startswith("мту")
    return "МТУ Росимущество" if interregional else "ТУ Росимущество"
...
# `им[ую]?ществ\w*`.
# ⚠️ Между «наследственное» и «имущество» — \s* (не \s+): суды склеивают
# ячейки карточки, и форма «наследственноеимущество» без пробела встречается
# в боевых данных (2-2898/2026, ХМАО). Квантификатор в первой альтернативе
# ЛЕНИВЫЙ (\w*?) — жадный \w+ съедал бы склейку целиком, и «им[ую]?ществ»
# уже не находилось.
...
def _shorten_heritage(name: str) -> str | None:
    ...
    # Пустые скобки — ДО разбора ФИО (21.08.2026). Карточка отдаёт «насл.
    # имущество Клешнина Маргарита Александровна ( )» (место под дату смерти
    # осталось незаполненным), и хвост «( )» ломал _match_fio: ФИО не
    # сворачивалось в инициалы, а скобки доживали до дайджеста — чистка
    # _EMPTY_PARENS_RE в _shorten_single стоит НИЖЕ раннего выхода наследства
    # и не отрабатывала. Одна строка чинит оба симптома: 52 симв. → 29.
    rest = _EMPTY_PARENS_RE.sub(' ', rest)
...
def _shorten_single(name: str, *, keep_fio_full: bool = False,
    ...
    if (_mtu_m := _MTU_HEAD_RE.match(name)):
        return _rosim_short(_mtu_m)
...
def shorten_party_name(name: str, *, keep_fio_full: bool = False) -> str:
    ...
    name = _MTU_FULL_RE.sub(_rosim_short, name)
...
def _norm_party_tokens(name: str) -> list[str]:
    ...
    collapsed = _MTU_FULL_RE.sub(_rosim_short, name)
