...
# С 18.08.2026 кэш ОБЩИЙ для обеих веток импортёра дампов: ответчик-строки
# пишут сюда карточный not_accepted (возврат/отказ в принятии/передача по
# подсудности из карточки — иначе повтор того же дампа качал бы карточку
# отказника заново). Коллизий ролей нет: роль фиксирована на дело, один и тот
# же (домен, номер) в двух ветках не встречается. Залипание при отмене возврата
# закрывает самоочистка в SEEN-гейте импортёра (строка выдачи больше не
# терминальная → запись снимается, карточка перечитывается).
PERMANENT_REJECTIONS = ("excluded_result", "excluded_writ", "no_link",
                        "already_spent", "not_accepted")
