...
def test_writ_not_expected_on_refusal_to_accept():
    """«Отказано в принятии» — листа не будет: дела ещё нет.

    ⚠️ До 14.08.2026 здесь стоял обратный тест: предполагалось, что у такого
    дела карточка отдаёт статус «Возвращено» и работает своя ветка завершения
    с окном 30 дней от события. Разгон Урала это опроверг — 9-125/2026
    (Пуровский районный) пришло со статусом «Решено», ушло в ветку «Решено без
    ИЛ» и 180 дней числилось в очереди на лист. Окно у обеих веток одинаковое
    (30 дней), а якорь ветки «листа не будет» доходит фолбэком до event_date —
    для карточки без решения и заседаний это единственная дата.
    """
    assert lifecycle.bank_writ_expected(
        {"result": "Отказано в принятии иска (заявления, жалобы)"}) is False
    # Боевая формулировка ГАС: шапка склеена с причиной без пробела.
    assert lifecycle.bank_writ_expected({
...
def test_writ_not_expected_on_return_and_transfer():
    """Возврат и передача по подсудности — тот же класс: листа не будет.

    Оба вида приходили со статусом «Решено» (9-31/2026 Берёзовского,
    2-1588/2026 и 2-8088/2026 Сургутских) и до 14.08.2026 ждали ИЛ 180 дней.
    Передачу исполняет суд, куда дело ушло, — под новым номером его подхватит
    обычный авто-подхват.
    """
    assert lifecycle.bank_writ_expected({
...
def test_empty_result_does_not_read_movement_history():
    """Пустой «Результат» — историю движения НЕ смотрим.

    `classify_fi_termination` при пустом результате уходит в events, а там у
    живого дела лежит отменённый возврат прошлого круга: передай мы туда
    события — дело молча перестало бы ждать лист. Тот же гейт, что в
    `fi_termination_details`.
    """
    assert lifecycle.bank_writ_expected({
        "status": "В производстве",
        "result": "",
        "last_event": "Возвращение искового заявления. 10:00",
        "events": [{"date": "01.03.2026",
                    "text": "Возвращение искового заявления заявителю"}],
    }) is True


def test_gp_return_in_merits_result_is_not_termination():
    """«Госпошлина возвращена» внутри исхода по существу — не завершение.

    Паттерны возврата якорятся существительным «заявление/иск/материал»
    (ревью 29.07.2026); страж от расширительного матча по слову «возвращ».
    """
    assert lifecycle.bank_writ_expected({
        "result": "Иск (заявление, жалоба) УДОВЛЕТВОРЕН, "
                  "госпошлина возвращена заявителю",
    }) is True
