...
class TestWorkflowWiring:
    ...
    def test_bank_counters_reach_operator(self):
        ...
        # Ярлыки — словами оператора (09.09.2026), а не именами фильтров.
        for word in ("без решения (возврат", "с уже выданным листом",
                     "уже отработали", "отсеяны ранее"):
    ...
    def test_repeat_is_promised_to_the_reserve_not_to_the_operator(self):
        """Повтор дампа с непрочитанными карточками УЖЕ стоит в очереди резерва
        (import_queue.jq берёт запись по fetch_fail/card_failed). Прежнее
        «повторите дамп, когда суд отвечает» писалось 16.08.2026, до появления
        очереди, и звало оператора делать сделанное. Ручной запасной выход
        обязан остаться: Mac бывает выключен, а пятничный дамп ждёт понедельника."""
        admin = _read_repo("cloudflare-worker/admin_page.js")
        assert "повторит локальная машина в течение дня" in admin
        assert "дочитает локальная машина или ближайший прогон" in admin
        assert "вставьте дамп заново" in admin, "убран ручной запасной выход"
        # Задача оператору осталась ровно там, где повтор бесполезен.
        assert "IMP_NO_COURT_RE" in admin
        assert "проверьте реестр региона (повтор не поможет)" in admin
