...
class TestAgentSchedules:
    ...
    def test_import_slots(self):
        """Расписание юриста (08.09.2026): будни 12:00–20:00 каждые 2 часа —
        зеркало VPS-таймера, который с этого дня ОСНОВНОЙ исполнитель импортов
        (до того 10:30–18:30 с 18.08.2026: облако часть теряло, резерв подбирал)."""
        assert _slots(IMPORT_PLIST) == {
            (w, h, 0) for w in range(1, 6) for h in (12, 14, 16, 18, 20)}
