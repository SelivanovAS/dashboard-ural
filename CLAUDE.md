...
- **Сводка** — `GET /admin/visits` (`handleAdminVisits`, [worker.js:2592](cloudflare-worker/worker.js:2592)), **только владельцу**: читает историю ОДНИМ `list` по префиксу и **без единого `get`** (`metadata` приходит вместе с ключами), пагинация по курсору ОБЯЗАТЕЛЬНА (бескурсорный `handleAdminImportLog` копировать нельзя — при >1000 ключей молча теряется хвост). Наружу уходит только огрызок `vid` (6 символов). ⚠️ Поллинга НЕТ и оператору не отдаётся: lists на free-tier тоже 1000/день, а операторов на Урале ~14.
...
- **Профиль-сирота раскрывается (29.08.2026, вопрос юриста «вижу 12 дел без
  ...
  косвенное: пересечение набора + кнопка 🔗 на самом устройстве (дашборд
  показывает там первые 8 символов id). Писать UA в профиль осознанно НЕ стали
